    ...
    <el-dialog title="危废订单发布"
      ...
      <el-form :model="ruleForm" :rules="rules"
        ...
        <el-form-item label="废物代码" prop="wasteCode">
          <el-select v-model="ruleForm.wasteCategoryCodeValue" style="width:40%;" placeholder="请选择危废类别"
        ...
        <el-form-item label="行业来源" prop="industrySourcesValue">
          <el-select v-model="ruleForm.industrySourcesValue"
                     @focus="handleGetProductionIndustryData"
                     style="width:40%;"
                     placeholder="请选择行业来源">
            <el-option
        ...
        <el-form-item label="危废来源" prop="wasteName">
          <el-input v-model="ruleForm.wasteName" placeholder="请填写危废来源" style="width:40%;"></el-input>
        </el-form-item>
        ...
        <el-form-item label="单位" prop="issueUnits">
          <el-select v-model="ruleForm.issueUnitValue"
                     placeholder="请选择一个单位"
                     style="width:40%;"
                     @focus="handleGetProductionIndustryData">
            <el-option
        ...
        <el-form-item v-if="ruleForm.whetherPricingValue==2" maxlength="10" label="单价" prop="unitPrice">
          <el-input v-model="ruleForm.unitPrice"
                    placeholder="请输入单价"
                    style="width:40%; margin-top:16px;"></el-input>
        </el-form-item>
        <el-form-item label="上传图片" class="upload-photos" required>
          <el-upload
            v-model="ruleForm.uploadPhotos"
            :action="$store.state.fileUploadUrl"
            :file-list="ruleForm.certificateAttIdUploadPhotos"
            list-type="picture-card"
            :before-remove="handleBeforeRemovePhotos"
            :before-upload="beforeAvatarUpload"
          ...
          <span style="color:#ddd;">请上传[1200*250]的图片</span>
        </el-form-item>
        <el-form-item label="危废介绍" prop="otherDescription">
          <el-input type="textarea" rows="4" v-model="ruleForm.otherDescription"
                    placeholder="请输入说明相关内容"></el-input>
        </el-form-item>
        <el-form-item label="危废图片" prop="uploadPhotos" class="upload-photos" required>
          <el-upload
            ...
            list-type="picture-card"
            ref="uploadPhotos"
            :file-list="ruleForm.wasteAttIdUploadPhotos"
            :limit="5"
            :on-exceed="handleOnExceed"
            :on-remove="handleBeforeWasteRemove"
            :before-remove="handleBeforeWasteRemovePhotos"
            :before-upload="beforeAvatarUpload"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="info" @click="handleOnlySaveOrder">暂存</el-button>
    ...
    data() {
      ...
      return {
        ...
        ruleForm: {
          ...
          defaultWasteCategoryValue: "",
          defaultIndustrySourcesValue: "",//行业来源--值
          wasteCategoriesCode: [],
    ...
    methods: {
      ...
      handleGetProductionIndustryData() {
        ...
        _this.$axios({
          ...
          url: "/base/findDictionary",
          data: {
            moduleCode: "hyly,unit"
          }
        }).then(res => {
          _this.ruleForm.industrySources = res.data.data.hyly;
          _this.ruleForm.issueUnits = res.data.data.unit;
        });
      },
      ...
      handleBeforeClose() {
        ...
        _this.ruleForm.certificateAttIdUploadPhotos = [];
        _this.ruleForm.wasteAttIdUploadPhotos = [];
      ...
      handleAvatarSuccess(res, file) {
        let _this = this;
        _this.ruleForm.imageUrl = _this.$store.state.outputImgByIdUrl + res.data;
        _this.ruleForm.imgLogoId = res.data;
      ...
      handleBeforeRemovePhotos(file) {
        let _this = this;
        _this.ruleForm.certificateAttIds.forEach((item, index) => {
          if (item == file.id || file.response.data)
            _this.ruleForm.certificateAttIds.splice(index, 1);
        });
        _this.$axios({
          method: "post",
      ...
      handleBeforeWasteRemovePhotos(file) {
        let _this = this;
        _this.ruleForm.fileIds.forEach((item, index) => {
          if (item == file.id || file.response.data)
            _this.ruleForm.fileIds.splice(index, 1);
        });
        _this.$axios({
          method: "post",
          url: "/attachment/delete",
      ...
      handleBeforeWasteRemove(file, fileList) {
        ...
        _this.$axios({
          ...
          if (res.data.status === 200) console.log(file.response.data + "删除成功！");
        })
      },
        ...
        _this.$axios({
          ...
          if (res.data.status === 200) {
            let {
              ...
              unit,//单位
              price,//单价
              uploadFileAttId,//上传图片
              remark,//危废介绍
              uploadTradeAttId,
              addressId//联系人id
            } = res.data.data;
            _this.ruleForm.title = daName;
            _this.ruleForm.imageUrl = screenAttId ? _this.$store.state.outputImgByIdUrl + screenAttId : '';
            _this.ruleForm.imgLogoId = screenAttId;
            _this.$axios({
              method: "post",
              withCredentials: true,
              url: "/base/findWasteType",
              ...
              if (res.data.status === 200) {
                res.data.data.map(item => {
                  if (item.extField == daCategory) {
                    _this.ruleForm.wasteCategoryValue = item.title;
                    _this.ruleForm.defaultWasteCategoryValue = item.extField
                  }
                });
              }
            });
            _this.ruleForm.wasteCategoryCodeValue = daCode;
            _this.ruleForm.wasteType = daType;
            _this.ruleForm.wasteName = dangerousWasteSource;
            _this.ruleForm.whetherPricingValue = priceMethod;
            _this.ruleForm.quantity = number;
            _this.$axios({
              method: "post",
              url: "/base/findDictionary",
              data: {
              ...
              if (res.data.status === 200) {
                res.data.data.hyly.map(item => {
                  if (item.itemCode == industrySource) {
                    _this.ruleForm.industrySourcesValue = item.title;
                    _this.ruleForm.defaultIndustrySourcesValue = item.itemCode
                  }
                });
                res.data.data.unit.map(item => {
                  if (item.itemCode == unit) {
                    _this.ruleForm.issueUnitValue = item.title;
                    _this.ruleForm.defaultIssueUnitValue = item.itemCode
                  }
                });
              }
            });
            _this.ruleForm.unitPrice = price;
            _this.ruleForm.certificateAttIds = uploadFileAttId ? uploadFileAttId : [];
            if (uploadFileAttId) {
              uploadFileAttId.forEach(item => {
                _this.ruleForm.certificateAttIdUploadPhotos.push({
                  id: item,
                  url: _this.$store.state.outputImgByIdUrl + item
                })
              });
            }
            _this.ruleForm.fileIds = uploadTradeAttId ? uploadTradeAttId : [];
            if (uploadTradeAttId) {
              uploadTradeAttId.forEach(item => {
                _this.ruleForm.wasteAttIdUploadPhotos.push({
                  id: item,
                  url: _this.$store.state.outputImgByIdUrl + item
                })
              });
            }
            _this.ruleForm.otherDescription = remark;
            _this.ruleForm.addressId = addressId;
      ...
      handleOnlySaveOrder() {
        let _this = this;
        if ((!_this.ruleForm.title && !_this.ruleForm.imgLogoId && !_this.ruleForm.wasteCategoryValue && !_this.ruleForm.wasteCategoryCodeValue && !_this.ruleForm.wasteType && !_this.ruleForm.industrySourcesValue && !_this.ruleForm.wasteName && !_this.ruleForm.whetherPricingValue && !_this.ruleForm.quantity && !_this.ruleForm.issueUnitValue && !_this.ruleForm.unitPrice && !_this.ruleForm.certificateAttIds.length && !_this.ruleForm.otherDescription && !_this.ruleForm.fileIds.length)) {
          _this.$message({
            type: 'warning',
            message: "请至少填写一个必填项！"
          });
          ...
          _this.$axios({
            ...
            data: {
              ...
              daCategory: /[\u4e00-\u9fa5]+$/.test(_this.ruleForm.wasteCategoryValue) ? _this.ruleForm.defaultWasteCategoryValue : _this.ruleForm.wasteCategoryValue,//危废类别
              daCode: _this.ruleForm.wasteCategoryCodeValue,//废物代码
              daType: _this.ruleForm.wasteType,//危废类型
              industrySource: /[\u4e00-\u9fa5]+$/.test(_this.ruleForm.industrySourcesValue) ? _this.ruleForm.defaultIndustrySourcesValue : _this.ruleForm.industrySourcesValue,//行业来源
              dangerousWasteSource: _this.ruleForm.wasteName,//危废来源
              priceMethod: _this.ruleForm.whetherPricingValue,//价格方式
              number: !_this.ruleForm.quantity ? "" : _this.ruleForm.quantity,//数量
              unit: /[\u4e00-\u9fa5]+$/.test(_this.ruleForm.issueUnitValue) ? _this.ruleForm.defaultIssueUnitValue : _this.ruleForm.issueUnitValue,//单位
              price: !_this.ruleForm.unitPrice ? '' : _this.ruleForm.unitPrice,//单价
              uploadFileId: _this.ruleForm.certificateAttIds.join(),//上传图片
              remark: _this.ruleForm.otherDescription,//危废介绍
              fileId: _this.ruleForm.fileIds.join(),//危废图片
        ...
        if (!_this.searchRuleForm.multipleSelection.length) {
          ...
          _this.$axios({
            ...
            data: {
              ...
              daName: _this.ruleForm.title,//标题
              screenFileId: _this.ruleForm.imgLogoId,// 年申报截图
              daCategory: /[\u4e00-\u9fa5]+$/g.test(_this.ruleForm.wasteCategoryValue) ? _this.ruleForm.defaultWasteCategoryValue : _this.ruleForm.wasteCategoryValue,//危废类别
              daCode: _this.ruleForm.wasteCategoryCodeValue,//废物代码
              daType: _this.ruleForm.wasteType,//危废类型
              industrySource: /[\u4e00-\u9fa5]+$/g.test(_this.ruleForm.industrySourcesValue) ? _this.ruleForm.defaultIndustrySourcesValue : _this.ruleForm.industrySourcesValue,//行业来源
              dangerousWasteSource: _this.ruleForm.wasteName,//危废来源
              priceMethod: _this.ruleForm.whetherPricingValue,//价格方式
              number: _this.ruleForm.quantity,//数量
              unit: /[\u4e00-\u9fa5]+$/g.test(_this.ruleForm.issueUnitValue) ? _this.ruleForm.defaultIssueUnitValue : _this.ruleForm.issueUnitValue,//单位
              price: !_this.ruleForm.unitPrice ? '' : _this.ruleForm.unitPrice,//单价
              uploadFileId: _this.ruleForm.certificateAttIds.join(','),//上传图片
              remark: _this.ruleForm.otherDescription,//危废介绍
              fileId: _this.ruleForm.fileIds.join(','),//危废图片
            ...
            if (res.data.status === 200) {
              ...
              _this.$parent.handleGetTableData();//加载表数据
