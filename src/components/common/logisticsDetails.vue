<template>
  <div class="logistics-detail">
      ...
      <el-row class="nav">
        ...
        <el-col :span="15" class="distribution">
          ...
          <el-col :span="12">
            ...
            <p>地址：{{consignee.provinceWaste+consignee.cityWaste+consignee.regionWaste+consignee.addressWaste}}</p>
            <p>种类：{{consignee.daCategory+'/'+consignee.daCode}}</p>
            <p>
              来源：{{consignee.industrySource}}{{consignee.dangerousWasteSource=='0'?'':'/'+consignee.dangerousWasteSource}}</p>
          </el-col>
        </el-col>
    ...
    <el-table class="bottom"
      ...
      <!--&lt;!&ndash;<el-button type="text" size="small">评价</el-button>&ndash;&gt;-->
      <!--&lt;!&ndash;<el-button type="text" size="small" @click="handleCollectionEnterprise(scope.row.companyId)">收藏企业&ndash;&gt;-->
    ...
    <div class="price" v-for="item in tableData" :key="item.id">
      <h3>付款信息：</h3>
      <p>商品总价：￥<span>{{item.number*item.offerTotal}}</span></p>
      <p>应付总额：￥<span>{{item.number*item.offerTotal}}</span></p>
    </div>
